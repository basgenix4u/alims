...
**The binding design contract.** The web application implements against this document — no guessing.

| Field | Value |
|---|---|
| **Document** | `ui_ux_specification.md` (repository root — placement per project-owner directive) |
| **Version** | 1.0.0 |
| **Author / Owner** | Abdulbasit Abdulalim |
...
2. **Change process.** Changes to this document ship as PRs against this file, in the same PR as the code that implements them.
...
| D8 | **Global-first, not local-first** | PRD §9.4 | English is first; every string lives in the i18n layer (the i18n layer), date/number formats are locale-aware, no culture-specific name/degree assumptions, NXR-ID rendered in a monospace face. |
...
`apps/web/src/styles/tokens.css` (this PR) exports every token below as a CSS custom property under `:root`. Components MUST consume tokens via variables (or the Tailwind mapping in §11) — **never raw hex values in component code**.
...
### 5.5 Upload experience (contract §8 of api_specification)
...
### 5.8 Registry & institution admin
...
### 5.10 Certificate & QR verification page (`GET /public/verify/:qrToken`)
...
Each component below lists anatomy, states, and non-negotiable a11y. These are built in `packages/ui` + `apps/web/src/components` as the token system lands.
...
## 7. Status & verification badge system
...
10. **Testing gate:** axe-core in CI with zero violations on new components; keyboard smoke script per journey.
...
- Zero webfonts; zero icon fonts; SVG inline; no client-rendered hero images; Next.js static/dynamic split per route .
...
3. **Existing scaffold cleanup:** replace `bg-surface-subtle`/`text-ink-muted` ad-hoc classes with mapped tokens; the placeholder `brand #1d4ed8` token is replaced by the §3 system.
4. **i18n:** all strings through the i18n layer from the first PR; this document's labels are the English source strings.
5. **Contracts:** component props typed from `@alims/contracts`; badge components take the enum union types directly — adding an enum value to `api_specification.md` requires a §7 mapping addition here.
...
- `apps/web`: root layout with skip-link, landing shell, health-check component (loading/error/success triad with non-colour-only errors), typed API client, Tailwind tokens mapped to the token system below.
- `packages/ui`: shell package, populated as the component specifications below are implemented.
...
**Gap list (resolved by this document + later subsystems):**
...
| G3 | No brand assets (logo, seal, favicon, OG) | §8.3 specs — SVG production |
| G4 | No status badge system; PRD §6.4 demands visibly distinct labels | §7 full enum→visual mapping |
...
| G9 | No accessibility test gate defined | §9.10 axe-core + keyboard smoke in CI |
...
|---|---|---|---|
| 1.0.0 | 2026-08-14 | Abdulbasit Abdulalim | Initial blueprint: brand system, verified tokens, per-screen navigation protocols, component specs, status badge mapping, accessibility contract, audit + gap list. |

**Status:** binding design contract for all frontend work.
