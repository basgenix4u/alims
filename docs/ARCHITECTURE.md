...
├─ docs/                   PRD.md · ARCHITECTURE.md · DECISIONS.md · design/
...
1. **Stage 1** — foundation: schema, API contract, monorepo scaffold, smoke endpoints
2. **Stage 2** — backend core: migrations, auth+MFA, policy engine, records/versions, audit chain
3. **Stage 3** — workflow, certificates, uploads, file safety pipeline
4. **Stage 4** — frontend: accessible responsive UI, auth flow, all screens wired to the live API
5. **Stage 5** — Docker, CI/CD, env config, deployment docs, release assets, tagged release
